...
pont$date <- as.POSIXct(date, format = "%d/%m/%Y %H:%M")
pont$jour <- format(pont$date, format="%d")
pont$mois <- format(pont$date, format="%m")
pont$annee <- format(pont$date, format="%y")
...
for(i in c(1:Nfourier))
{
	...
	eq[[i]]<-as.formula(paste("Load.detrend~",fourier,sep=""))
...
par(mfrow=c(1,2))
plot(pont$Load.detrend,type='l', lwd=2)
...
plot(pont$Load.detrend[1:52],type='l', lwd=2)
